...
axis_xt = [0 10 0 10 0 1];
RHO_0 = [0.1, 0.9, 0.2, 0.4, 0.7, 0.9]';
X0_0 = [1, 2, 3, 4, 5]';
...
for i=1:length(RHO0)-1
    rho1=RHO0(i,1);
    Q_rho1=calculate_Q_rho(rho1,v_f,rho_max,Q_max);
    rho2=RHO0(i+1,1);
    Q_rho2=calculate_Q_rho(rho2,v_f,rho_max,Q_max);

    if Q_rho1>Q_rho2
        lamda(i,1)=calculate_lamda(rho1,rho2,v_f,rho_max,Q_max);
    elseif Q_rho1<Q_rho2
        ...
        for j = 1:num
            ...
            lamda_wave2(j,k) = calculate_lamda(rho_wave2(j,k), rho_wave2(j+1,k),v_f,rho_max,Q_max);
        end
        k=k+1;
    end    
end
...
for i=1:length(RHO1)-1
    rho1=RHO1(i,1);
    rho2=RHO1(i+1,1);
    lamda1(i,1) = calculate_lamda(rho1,rho2,v_f,rho_max,Q_max);
end

%% lamda1 delete repeat 
matrix=lamda1;
% 找到第一列中的相邻重复项
repeated_indices = find(diff(matrix(:,1)) == 0);

% 合并相邻重复项
merged_matrix = matrix;
merged_matrix(repeated_indices+1,:) = [];

% 记录删除重复项的位置
deleted_indices = repeated_indices + 1;

lamda1_delete_repeat=merged_matrix;
lamda1=lamda1_delete_repeat;

%% x_min1 delete repeat
x_min1_delete_repeat=x_min1;
x_min1_delete_repeat(deleted_indices,:) = [];
x_min1=x_min1_delete_repeat;

%% RHO1 delete repeat
RHO1_delete_repeat=RHO1;
RHO1_delete_repeat(deleted_indices,:) = [];
RHO1=RHO1_delete_repeat;
...
t_min2_location = find(T==t_min2);

idx=0;

for i=1:length(T)
    if abs(t_min2-T(i,1))<0.00001
        idx=idx+1;
    end
end
...
if idx>1
    for i=1:idx
        t_location(i,1) = t_min2_location+i-1;
    end
...
draw_2D_figures(lamda1, t_min1, t_min2, x_min1, t_step_size);
...
for i = 1:length(RHO2)-1
    rho1=RHO2(i,1);
    rho2=RHO2(i+1,1);
    lamda2(i,1) = calculate_lamda(rho1, rho2,v_f,rho_max,Q_max);
end
...
if length(RHO2)>2
    ...
    t_min3_location = find(abs(T-t_min3)<0.0001);
...
draw_2D_figures(lamda2, t_min2, t_min3, x_min2, t_step_size)
...
if length(RHO3)>1
    ...
    while length(RHO) > 1
        ...
        if length(lamda) > 1
            ...
            t_min_f_location = find(T == t_min_f);
        else
            t_min_f = axis_xt(4);
        ...
        % draw 2-D figures
        draw_2D_figures(lamda, t_min_o, t_min_f, x_min_o, t_step_size)
